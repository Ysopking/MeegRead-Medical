...
def load_edf(path: str | Path) -> Tuple[Dict[str, List[float]], float, int, List[str]]:
    ...
    with open(path, "rb") as handle:
        _read_ascii(handle, 8)       # version
        _read_ascii(handle, 80)      # patient
        _read_ascii(handle, 80)      # recording
        _read_ascii(handle, 8)       # date
        _read_ascii(handle, 8)       # time
        ...
        headers = [EdfSignalHeader(
            label=labels[i].strip() or f"CH{i+1}", unit=units[i].strip() or "a.u.",
            physical_min=pmin[i], physical_max=pmax[i], digital_min=dmin[i], digital_max=dmax[i],
...
def metrics(scales: Sequence[core.ScaleResult]) -> Dict[str, float]:
    ...
    return {
        "meanPhaseLocking_slope": ols_slope(plv),
        "kappa_slope": ols_slope(kappa),
        "logQ_slope": ols_slope(log_q),
    }
...
def run_subject(args: argparse.Namespace) -> None:
    ...
    summary = {
        ...
        "null": {
            ...
            "meanPhaseLocking_slope_median": statistics.median(null_plv),
            "kappa_slope_median": statistics.median(null_kappa),
            "logQ_slope_median": statistics.median(null_logq),
        },
        "inference": {
            "primary_empirical_p": empirical_p_greater(observed["meanPhaseLocking_slope"], null_plv),
            "secondary_kappa_empirical_p": empirical_p_greater(observed["kappa_slope"], null_kappa),
            "secondary_logQ_empirical_p": empirical_p_less(observed["logQ_slope"], null_logq),
        },
        "guardrail": "SENSOR_SPACE_PROXY; implementation/statistical validation only; no diagnostic, biomarker, source-localization, causal or universality claim"
    }
...
def run_aggregate(args: argparse.Namespace) -> None:
    ...
    for subject in SUBJECTS:
        ...
        rows.append({
            "subject": subject,
            "primary_slope": obj["observed"]["meanPhaseLocking_slope"],
            "primary_null_median": obj["null"]["meanPhaseLocking_slope_median"],
            "primary_p": obj["inference"]["primary_empirical_p"],
            "kappa_slope": obj["observed"]["kappa_slope"],
            "kappa_p": obj["inference"]["secondary_kappa_empirical_p"],
            "logQ_slope": obj["observed"]["logQ_slope"],
    ...
    group = {
        ...
        "subjects": SUBJECTS,
        "n": len(rows),
        "primary_subjects_p_le_0_05": significant,
        "primary_group_binomial_tail_p": binomial_tail(significant, len(rows), 0.05),
        "median_observed_primary_slope": statistics.median(float(r["primary_slope"]) for r in rows),
        "median_subject_null_primary_slope": statistics.median(float(r["primary_null_median"]) for r in rows),
        "group_alpha": 0.05,
        "guardrail": "Confirmatory operator-level replication relative to preregistered circular-shift null only; not a physical-law, clinical, causal or source-space claim"
    }
